...
\glsreset{swb}
In diesem Kapitel wird der Einfluss erläutert, wie sich \textit{\gls{sm}} auf das \textit{\gls{swb}} auswirken. Dazu werden verschieden Studien untersucht und mittels \textit{\gls{cm}} grafisch zusammengeführt. Der Einfluss auf das \textit{\gls{swb}} ist von verschiedenen Faktoren abhängig, welche in \textit{Abbildung~\ref{fig.ConceptMapSwbSm}} ersichtlich sind.\newline
Die Grafik stellt den Zusammenzug unterschiedlicher Studien dar, die sich mit den Auswirkungen von \textit{\gls{sm}} auf das \textit{\gls{swb}} auseinander setzen. Darin sind die verschiedenen Konstrukte ersichtlich, die in den Studien verwendet wurden. Gelesen wird die Grafik ausgehend von den \textit{\gls{sm}} entlang den Verbindungslinien bis hin zum  \textit{\gls{swb}}.\newline
In den folgenden Unterkapiteln werden diese Konzepte anhand der Grafik erläutert. Dazu wird die Grafik in verschiedene Stränge unterteilt, um den komplexen Informationsgehalt in übersichtliche Häppchen aufzuteilen. Die Stränge sollen von den \textit{\gls{sm}} ausgehend, über wichtige Konzepte und Begriffe, bis hin zum \textit{\gls{swb}} führen. \newline
...
\end{figure}
%UK 
\section{\textit{self-presentation} und \textit{self-disclosure}}\label{sub.selfp}
